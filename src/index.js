        ...
        {lvlel.map((dayname)=>(
          <div id='sorted' className='sorted'>
            ...
            {dayname.groups.filter(g => g.name === group).map((group) =>
              splitSubjects(group.subjects[0]).map((subject)=> (
...
function App() {
  ...
  if (currentPage === "first") {
    return <TablePage lvlel={mainbaseffirst} onClickBack={() => setCurrentPage("main")} />;
  }
...
somebase.map((elem)=>{
  if(elem.date=="2021-03-10"){
    somemainebase[0].groups[elem.group-1].name==numberBaseForGroup[elem.group-1]
    somemainebase[0].groups[elem.group].subjects[elem.period]==numberBaseForSubject.elem.subject
  }
})
